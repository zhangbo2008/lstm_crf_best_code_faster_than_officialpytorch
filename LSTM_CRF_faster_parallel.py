import time #这个是crf最优版本. 读这个进行学习. 任务就是经典的NER任务.
import torch
import torch.nn as nn
import torch.optim as optim
#==========这个核心loss函数,的细节在 https://zhuanlan.zhihu.com/p/372023947
START_TAG = "<START>"
STOP_TAG = "<STOP>"
...
class BiLSTM_CRF_MODIFY_PARALLEL(nn.Module):
    ...
    def __init__(self, vocab_size, tag_to_ix, embedding_dim, hidden_dim):
        ...
        # transitioning *to* i *from* j.      #注意这里面是从j 到i 的转移概率
        self.transitions = nn.Parameter(#============注意区分torch里面几个变量的设置 Parameter是tensor子类.
...
#保证跳转合法性.
    ...
    def _forward_alg_new(self, feats):# 对所有的label组合求概率和.也就是归一化因子.
        # Do the forward algorithm to compute the partition function
        ...
        for feat_index in range(feats.shape[0]):  # -1
            ...
            forward_var_list.append(torch.logsumexp(aa, dim=1)) # 因为我们计算当前到达位置的exp得分和.所以我们用logsumexp,每次都需要做这种互逆运算因为上面一行加和是在log维度里面的.但是求和是在exp维度的.所以sumexp之后还log回去.
    ...
    def _forward_alg_new_parallel(self, feats): #feats是encoder的特征.
        # Do the forward algorithm to compute the partition function
        init_alphas = torch.full([feats.shape[0], self.tagset_size], -10000.)#.to('cuda')
        # START_TAG has all of the score.
        init_alphas[:, self.tag_to_ix[START_TAG]] = 0. # 初始化概率分布init_alphas
        ...
        for feat_index in range(feats.shape[1]):  # -1
            ...
            # gamar_r_l = torch.transpose(gamar_r_l,0,1)
            t_r1_k = torch.unsqueeze(feats[:, feat_index, :], 1).transpose(1, 2)  # +1 encoder 的概率
            # t_r1_k = feats[:,feat_index,:].repeat(feats.shape[0],1,1).transpose(1, 2)
            aa = gamar_r_l + t_r1_k + torch.unsqueeze(self.transitions, 0) # 更新用的上次概率加encoder概率加transition概率.    aa[i][j]表示累积的当前最后一步是从j到i的累积概率
            # forward_var_list.append(log_add(aa))
            forward_var_list.append(torch.logsumexp(aa, dim=2)) # 在最后一个维度上进行求和.这个地方是跟vitebi的区别,vitebi算的是max, 这里面算的是sum
        terminal_var = forward_var_list[-1] + self.transitions[self.tag_to_ix[STOP_TAG]].repeat([feats.shape[0], 1])
        # terminal_var = torch.unsqueeze(terminal_var, 0)
        alpha = torch.logsumexp(terminal_var, dim=1) #最后这个在第一个维度上再sum一遍.得到整个序列的置信度.
        return alpha
    ...
    def _score_sentence_parallel(self, feats, tags):
        ...
        tags = torch.cat([torch.full([tags.shape[0],1],self.tag_to_ix[START_TAG], dtype=torch.long),tags],dim=1)#在tags前面加上start_tag.
        for i in range(feats.shape[1]):
            feat=feats[:,i,:] # encoder 概率,下面一行计算跟vitebi一样.都是encoder概率加上转移概率.一直加就计算了上一轮累计值.
            score = score + \
    ...
    def _viterbi_decode_new(self, feats):#更好的vitebi算法. =====feats表示的是每一步通过encoder得到的ner概率.
        backpointers = []

        # Initialize the viterbi variables in log space=====前向传导.目前到start_tag的概率是1,所以就是0.因为是log空间的.这个数值exp之后是概率. 所以exp0=1.表示概率是1.
        init_vvars = torch.full((1, self.tagset_size), -10000.)#.to('cuda')
        init_vvars[0][self.tag_to_ix[START_TAG]] = 0

        # forward_var at step i holds the viterbi variables for step i-1
        forward_var_list = [] # 维护的是调转概率.
        forward_var_list.append(init_vvars)

        for feat_index in range(feats.shape[0]):
            gamar_r_l = torch.stack([forward_var_list[feat_index]] * feats.shape[1]) # 扩充6遍.表示用之前的跳转到下一个全部的label概率一样.
            gamar_r_l = torch.squeeze(gamar_r_l)
            next_tag_var = gamar_r_l + self.transitions # 上一个分布加上转移矩阵等于下一个分布#这个动态规划矩阵Aij含义是 从j到i行的最大累计概率. 得到decoder的概率.
            # bptrs_t=torch.argmax(next_tag_var,dim=0)
            viterbivars_t, bptrs_t = torch.max(next_tag_var, dim=1) # viterbivars_t 记录了当前到达各个位置的最优距离. dim=1是因为计算的是全部的to ,所以要对from做max.

            t_r1_k = torch.unsqueeze(feats[feat_index], 0) # 表示encoder得到的概率分布#这行放在上一行的下面是因为.第一个位置start是没有feats特征的.
            forward_var_new = torch.unsqueeze(viterbivars_t, 0) + t_r1_k

            forward_var_list.append(forward_var_new)
            backpointers.append(bptrs_t.tolist())

        # Transition to STOP_TAG========最后一次调转到stop_tag.计算道理同start一样,同样没有feats,如果有stat和stop特征,加上即可.
        terminal_var = forward_var_list[-1] + self.transitions[self.tag_to_ix[STOP_TAG]] # 还是积累概率加上调转概率但是这一次因为stop_tag没有encoder的概率分布所以不用加了.
        best_tag_id = torch.argmax(terminal_var).tolist()
        path_score = terminal_var[0][best_tag_id]

        # Follow the back pointers to decode the best path. # 根据back指针得到路径.
        best_path = [best_tag_id]
        for bptrs_t in reversed(backpointers):
            best_tag_id = bptrs_t[best_tag_id] #因为我们计算的是from的索引.所以是逆序.
            best_path.append(best_tag_id)
        # Pop off the start tag (we dont want to return that to the caller)
        start = best_path.pop() #第一个是start我们不用.
        assert start == self.tag_to_ix[START_TAG]  # Sanity check
        best_path.reverse()
        return path_score, best_path
#==========这个核心loss函数,的细节在 https://zhuanlan.zhihu.com/p/372023947
    def neg_log_likelihood(self, sentence, tags):
        ...
        forward_score = self._forward_alg_new(feats) # 对应上面博客里面的分母
        gold_score = self._score_sentence(feats, tags)[0] # 对应博客里面的分子.
        return forward_score - gold_score
#====这个核心loss函数,的细节在 https://zhuanlan.zhihu.com/p/372023947
    def neg_log_likelihood_parallel(self, sentences, tags):
        feats = self._get_lstm_features_parallel(sentences)
        forward_score = self._forward_alg_new_parallel(feats) # 对应上面博客里面的分母
        gold_score = self._score_sentence_parallel(feats, tags) # 对应博客里面的分子.
        return torch.sum(forward_score - gold_score) # 目标让这2个接近即可.

    def forward(self, sentence):  # dont confuse this with _forward_alg above.
        # Get the emission scores from the BiLSTM
        lstm_feats = self._get_lstm_features(sentence) # 使用lstm生成概率. shape= seq_len, label_nums

        # Find the best path, given the features.
        score, tag_seq = self._viterbi_decode_new(lstm_feats)
        return score, tag_seq

if __name__ == '__main__':# bert--linear, bert--crf, blstm-crf
    START_TAG = "<START>"
    STOP_TAG = "<STOP>"  #这2个crf里面用.
    PAD_TAG = "<PAD>"
    EMBEDDING_DIM = 300
    HIDDEN_DIM = 256
    ...
    sentence_in_pad, targets_pad = prepare_sequence_batch(training_data, word_to_ix, tag_to_ix)  # 这个不用每次都算.
    ...
    print("训练之后我们看一下效果.")
    print('测试句子是',training_data)
    print("label",tag_to_ix)
    with torch.no_grad():
        precheck_sent = prepare_sequence(training_data[0][0], word_to_ix)
        print(model(precheck_sent))
